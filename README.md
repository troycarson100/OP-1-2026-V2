...
**Your own audio:** Use **LOAD** or drag a WAV / AIFF / FLAC / OGG file onto the editor window; the file is imported into the **currently selected** track and resampled to the host rate (imports are capped at **30 minutes** per file to keep memory predictable). On the **MATERIAL** page, **Playhead** scrubs the tape (clamped to the loop); drag it during playback to jump in real time until you release (the tape read head slews toward the knob so scrubbing stays smooth). **Wave Zoom** magnifies the waveform around that focus (playhead while stopped, live tape while playing, or the playhead again while you are dragging it). The **Input Capture** knob arms recording from the **plugin input** (enable an input in the standalone or DAW and turn the knob past halfway while the track is playing); the capture buffer wraps circularly over an **~8 second** material window. In the **standalone** app, open the audio device settings and choose an input if you want to capture from a microphone or interface. Longer capture notes appear in the help line's tooltip.
