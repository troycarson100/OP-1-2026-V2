...
public:
    ...
    // Append PCM to the next free pool slot. right may be null (mono → duplicated). Returns the
    // slot index it landed in, or -1 if the pool is full. Message thread only.
    int addSample (const float* left, const float* right, int numFrames, const char* name);

    // Place PCM into a specific absolute slot (used by state restore). Message thread only.
    void loadSlot (int slot, const float* left, const float* right, int numFrames, const char* name);
